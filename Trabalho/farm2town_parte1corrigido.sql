...
-- 12º relatório
-- Barbara Ramos
-- Listar os entregadores que realizaram entregas, e têm avaliações
...
UNION
(SELECT p.cod_pedido, c.nome AS cliente_nome, NULL AS entregador_nome, p.valor_pedido, p.data_pedido
FROM pedido p
LEFT JOIN entrega en ON p.cod_pedido = en.cod_pedido
LEFT JOIN cliente c ON en.cod_cliente = c.cod_cliente
WHERE en.cod_entregador IS NULL);
